#!/usr/bin/perl -w
# Copyright (C) 2014 Daryl W. Grunau
#
...
use POSIX qw(:errno_h :sys_wait_h times tmpnam);
...
our $opt_a;				# all
our $opt_debug;				# debug
our $opt_fanout=256;			# remote-copy fan-out
our $opt_h;				# help
our $opt_l;				# login
our $opt_m;				# machines
our $opt_man;				# man page
our $opt_output='-';			# output file (default: STDOUT)
our $opt_P;				# parallel
our $opt_p;				# prefix
our $opt_rand;				# randomize host list
our $opt_rcp;				# rcp to remote
our $opt_opts;				# remote copy program options
our $opt_ping;				# ping remote before exec'ing cmd
our $opt_ping_host;			# ping remote:port/proto
our $opt_ping_timeout='5';		# ping timeout (default: 5 sec)
our $opt_rsh;				# rsh remote
our $opt_rsync;				# rsync to remote
our $opt_rsync_rsh;			# rsync remote transport shell
our $opt_s;				# skip
our $opt_scp;				# scp to remote
our $opt_ssh;				# ssh remote
our $opt_success='0';			# successful exit statuses
our $opt_t=300;				# timeout
our $opt_v;				# verbose
our $opt_V;				# verify
...
our $strand = '';			# internally-generated random string
...
local $SIG{'TSTP'} = \&tstp_signal_handler; our $SIGTSTP = 0;
...
H: while (@MachineList) {
   ...
   while (@WorkQ) {
	 ...
	 : shift(@MachineList)
      ...
      if ( my $pid = &spawn($machine,$sender) ) {
	 $RUNpids{$pid} = {
...
sub spawn ($$) {
   ...
   if ( $pid = fork ) {				# parent
	 ...
	 warn " '$cmd': command timed out ($opt_t sec)\n";
	 exit ETIMEDOUT;
      };
...
sub GetMachines ($$) {
   ...
   my @Machines = ();
   my $comma = ',(?![\d,-]+\])';# match a comma that is NOT followed by
				# digits|commas|hyphens & ']' (a negative
...
M:    foreach my $name (split(/$comma/,$machines)) {
	 ...
	 if ($name =~ /^\@([^\@]+)$/) {			# a lone netgroup
	    ...
	    if ($chunkL eq $chunkR) {
	       push(@Machines,$chunkL);
	       next M;
	    }

	    my ($prefix, $numberL, $numberR, $suffix) =
	       @{ ParseRange($chunkL, $chunkR) || (push(@Machines,$name), next M)};

	    if ($numberL <= $numberR) {
	       my $fmt = ($numberL =~ /^0/
		  ? "$prefix%0".length($numberL)."d$suffix"	# a leading zero
		  : "$prefix%d$suffix"
	       );
		  ...
		  push(@Machines,sprintf("$fmt",$i));
	       ...
	       my $fmt = ($numberR =~ /^0/
		  ? "$prefix%0".length($numberR)."d$suffix"	# a leading zero
		  : "$prefix%d$suffix"
	       );
	       for (my $i=$numberL; $i >= $numberR; $i--) {
		  push(@Machines,sprintf("$fmt",$i));
   ...
   my %SEEN = ();
   my @Unique = grep { ! $SEEN{$_}++ } @Machines;
   return @Unique;
}

# Seek a common prefix 'p' and suffix 's' between two *differing* string
# segments of the form 'pN1s' and 'pN2s'.  Return an array reference
# containing 'p', 'N1', 'N2' and 's'.
sub ParseRange ($$) {
   my ($cL, $cR) = @_;

   my $nL = my $nR = my $pX = my $sX = undef;

   $pX = $cL;
   $pX = substr($pX, 0, -1) while ($cR !~ /^\Q$pX\E/);
   $pX =~ s/(\d+)$//;	# some numerical pad is part of the prefix!

   $sX = $cL;
   $sX = substr($sX, 1) while ($cR !~ /\Q$sX\E$/);
   $sX =~ s/^(\d+)//;	# some numerical pad is part of the suffix!

   if ($cL =~ /^\Q$pX\E(\d+)\Q$sX\E$/) {
      $nL = $1;
   } else {
      return undef;
   }

   if ($cR =~ /^\Q$pX\E(\d+)\Q$sX\E$/) {
      $nR = $1;
   } else {
      return undef;
   }

   if ($nL == $nR) {	# numeric equality
      return (length($nL) > length($nR)
	 ? [$pX, $nL, $nL, $sX]
	 : [$pX, $nR, $nR, $sX]
      );
   } else {
...
sub SlurmLoadNode {
   return undef unless (eval "require Slurm");
   Slurm->import(qw(:constant));
...
5)  % pexec -pP -m @compute --ping --ping-host %host%-bmc:echo/icmp
powerman --cycle %host%

Power cycle nodes defined by the compute netgroup predicated by the
successful echo/icmp ping to the host's BMC interface.
